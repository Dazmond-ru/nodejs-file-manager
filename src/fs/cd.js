...
export const cd = async (prevPath, chunkStringified, isDirectoryCheck) => {
  ...
  if (
    inputPathNormalize[inputPathNormalize.length - 1] === path.sep &&
    inputPathNormalize.length !== 1
  )
  ...
  if (isDirectoryCheck) {
    if (/^[A-Za-z]:{1}/.test(inputPath)) {
      ...
      if (inputPath.length === 2 && inputPath[inputPath.length - 1] === ":")
        inputPath = inputPath + `${path.sep}`;
      try {
        chdir(inputPath);
        return inputPath;
      } catch (err) {
